...
public class ImageSearch {
	...
	private static final String datasetpath = ".." + File.separator + "ImageData" + File.separator + "train"
			+ File.separator + "data"; // training image set // dataset
	private static final String groundTruthsPath = ".." + File.separator + "Groundtruth" + File.separator + "train"; // groundtruths
																														// //
																														// file
	private static final String imageListPath = ".." + File.separator + "ImageList" + File.separator + "train"
			+ File.separator + "TrainImagelist.txt"; // image list // file
	private static final String imageTagsPath = ".." + File.separator + "ImageData" + File.separator + "train"
			+ File.separator + "train_tags.txt"; // image tags // tags
	...
	private void calculateSimilarities(List<SearchType> searchTypes, ImageData queryImage) throws IOException {
		for (SearchType searchType : searchTypes) {
			...
			case FEATURE:
				VisualConcept.computeSimilarity(new ArrayList<ImageData>(images.values()), queryImage);
				break;
	...
	private List<ImageData> pseudoRelevanceFeedback(List<SearchType> searchTypes, List<ImageData> results,
			List<ImageData> feedback) throws IOException {
		...
		for (ImageData id : feedback) {
			...
			for (ImageData fid : feedbackResults) {
				...
				pseudoRanks.put(fid.getFilename(), pseudoRanks.get(fid.getFilename())
						+ (1.0 - ((rank / feedbackResults.size())) / feedback.size()));
				rank++;
			}
		}
			...
			for (File folder : new File(datasetpath).listFiles()) {
				...
				for (int count = 0; count < files.length; count++) {
					if (!Utils.getExtension(files[count]).equals("sift")
							&& !Utils.getExtension(files[count]).equals("txt")) {
						String filename = files[count].getName();
						ImageData id = new ImageData(filename, files[count].getPath(), tags.get(filename));
